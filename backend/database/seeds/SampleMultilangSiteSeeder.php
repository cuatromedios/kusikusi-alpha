...
class SampleMultilangSiteSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        $home->content = [
            "title" => [ "en" => "Kusikusi Website", "es" => "Sitio web Kusikusi"],
            'welcome' => [ "en" => "Welcome to your new website", "es" => "Bienvenido a tu nuevo sitio web"],
            "slug" => [ "en" => "", "es" => "es"],
        ];
        $home->save();
        for ($s = 0; $s < $sections_count; $s++) {
            $section = factory(App\Models\Entity::class)->states('multilang')->make([
                "model" => "section",
                "parent_entity_id" => $home->id
            ]);
            $section->save();
            for ($p = 0; $p < $pages_count; $p++) {
                $page = factory(App\Models\Entity::class)->states('multilang')->make([
                    "model" => "page",
